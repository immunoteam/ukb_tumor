---
title: "L_102_Coy_models_survival_analysis_oa"
author: "Bagi Laura"
date: "2024-11-19"
...
surv_resdf_cau_maf = bind_rows(res)
save(surv_resdf_cau_maf, file = "objects/res_surv_overall_cau_maf")
...
#Load df
load("objects/res_surv_overall_cau_maf")
...
png(filename = "plots/Survival/surv_coxMAF_overall_heatmap.png", width = 30, height = 20, units = "cm", res = 300)
...
tumorous_all = ukb_data %>% 
      filter(cancer_type %in% tumors) %>% 
    dplyr::select(eid, birthdate, sex, death, date_of_death, age_at_cancer, diag_date, cancer_type, death_type, metastati_tumor, synonym_burden, all_of(ptvs), all_of(gpca))
nottumorous_all = ukb_data %>% 
      filter(cancer_type == "No_cancer") %>% 
    dplyr::select(eid, birthdate, sex, death, date_of_death, age_at_cancer, diag_date, cancer_type, death_type, metastati_tumor, synonym_burden, all_of(ptvs), all_of(gpca))
...
res = pblapply(ptvs_maf, function(p) {
  i=i+1
  .GlobalEnv$i = i
  pdf_filename = paste0(out_dir, "forest_surv_MAF_oa_", p, ".pdf")
...
res_surv = pblapply(ptvs_maf, function(p) {
  i=i+1
  .GlobalEnv$i = i
  pdf_filename = paste0(out_dir, "surv_plot_maf_oa_", p, ".pdf")
  pdf(pdf_filename, onefile = T, width = pdf_width, height = pdf_height)
  tumors_survres = pbsapply(tumor_freq_data$cancer_type, function(t) {
    tumorous_d = tumorous_all %>% 
      filter(cancer_type == t & death == T) %>% 
...
pblapply(tumors, FUN = function(t){  
  pdf_filename = paste0(out_dir, "TIC_", gender, "_forest_", t, ".pdf")
  pdf(pdf_filename, onefile = T, width = pdf_width, height = pdf_height)
...
pblapply(tumors, FUN = function(t){  
  ...
  pdf(pdf_filename, onefile = T, width = pdf_width, height = pdf_height)
  fig = fun_TumorSurvPlotOS(tumor = t, gender = gender, geneset = geneset, ptv_burden_cat = T, dataset_dir = "C:/Users/bagil/Desktop/MyGit/ukb_tumor/objects/000_Sub_dataset/", ptv_dir = "C:/Users/bagil/Desktop/MyGit/ukb_tumor/objects/PTVvars/")
  print(fig)
  dev.off()
})
