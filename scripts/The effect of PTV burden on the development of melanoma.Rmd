...
ukb_data_melanoma %<>% mutate(melanoma = T, .after = eid)

nonmelanoma_IDs = unique(setdiff(ukb_data_cancer$eid, melanoma_IDs))
ukb_data_nonmelanoma = ukb_data_cancer %>% filter(eid %in% nonmelanoma_IDs) %>% arrange(diag_date)
ukb_data_nonmelanoma = ukb_data_nonmelanoma[fmatch(nonmelanoma_IDs, ukb_data_nonmelanoma$eid),]
ukb_data_nonmelanoma %<>% select(-c(attending_2, attending_3, attending_4, cancer_type, number_of_MAF_1_percent, PTV_burden_of_MAF_1_percent, shet_burden_of_MAF_1_percent))
ukb_data_nonmelanoma %<>% mutate(melanoma = F, .after = eid)

ukb_data = rbind(ukb_data_melanoma, ukb_data_nonmelanoma)
rm(ukb_data_cancer, ukb_data_melanoma, ukb_data_nonmelanoma, melanoma_IDs, nonmelanoma_IDs)
...
set.seed(123)
ukb_data$melanoma = as.factor(ukb_data$melanoma)
...
glm = h2o.glm(x = c("age", "ethnic_group", "s.color", paste0("gpca_", 1:10), "PTV_burden"), y = "melanoma", training_frame = train)
glm@model

glm2 = h2o.glm(x = c("age", "ethnic_group", "s.color", paste0("gpca_", 1:10), "PTV_universal_1"), y = "melanoma", training_frame = train)
glm2@model

# Run AutoML for 20 base models
aml <- h2o.automl(x = c("age", "ethnic_group", "s.color", paste0("gpca_", 1:10), "PTV_burden"), y = "melanoma",
