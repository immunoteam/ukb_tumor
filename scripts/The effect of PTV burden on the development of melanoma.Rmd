...
icd10s = ukb_data_cancer %>% group_by(ICD10_codes) %>% summarise(n = n()) %>% filter(n > 500) %>% pull(ICD10_codes)
res_p_or = future_sapply(icd10s, function(x) {
  cancer_tbl = ukb_data_cancer %>% filter(ICD10_codes == x) %>% arrange(diag_date)
  cancer_tbl = cancer_tbl[!duplicated(cancer_tbl$eid),]
  cancer_tbl %<>% mutate(cancer = T, .after = eid)
  ukb_data = rbind(cancer_tbl, nocancer_tbl)
  ukb_data$cancer = as.factor(ukb_data$cancer)
  if(nrow(table(ukb_data$PTVtsg>0, ukb_data$cancer)) == 2 & ncol(table(ukb_data$PTVtsg>0, ukb_data$cancer)) == 2) {
    ft = fisher.test(table(ukb_data$PTVtsg>5, ukb_data$cancer))
...
res_p_or$V2 = as.numeric(res_p_or$V2)
