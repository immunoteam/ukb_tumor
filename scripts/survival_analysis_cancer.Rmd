...
#Load file
ukb_data = readRDS("objects/ukb_data.rds")
sukb_data$cancer_type[ukb_data$cancer_type == "Brain/CNS"] = "Brain"
ukb_data$cancer_type[ukb_data$cancer_type == "Hodgkin lymphoma"] = "HodgkinLymphoma"
ukb_data$cancer_type[ukb_data$cancer_type == "Multiple myeloma"] = "MultipleMyeloma"
ukb_data$cancer_type[ukb_data$cancer_type == "Non-Hodgkin lymphoma"] = "NonHodgkinLymphoma"
ukb_data$sex = factor(ukb_data$sex, levels = c(0,1), labels = c("female", "male"))
...
res = pblapply(ptvs, function(p) {
  ...
  tumors_survres = pbsapply(X = tumor_freq_data$cancer_type, FUN = function(t) {
    ...
    tempdfU %<>% dplyr::select(eid, time, event, sex, all_of(p), gpca1, gpca2, gpca3, gpca4, gpca5, gpca6, gpca7, gpca8, gpca9, gpca10)
    colnames(tempdfU)[5] = "PTVvar"
    res.surv = coxph(Surv(time, event) ~ PTVvar + sex + gpca1 + gpca2 + gpca3 + gpca4 + gpca5 + gpca6 + gpca7 + gpca8 + gpca9 + gpca10, data = tempdfU, id = tempdfU$eid)
    ...
    coxPvalues = coxPvalues[fmatch(c("PTVvar", paste0("gpca", 1:10)), names(coxPvalues))]
    coxCoefs = coxCoefs[fmatch(c("PTVvar", paste0("gpca", 1:10)), names(coxCoefs))]
...
save(surv_resdf, file = "objects/res_surv_all")
...
library(dendsort)
#Load df
load("objects/res_surv_all")

#Heatmap

tumors = unique(surv_resdf$cancer_type)
ptvs = unique(surv_resdf$ptv_type)
...
for(i in 1:nrow(dataForHm)) {
  for(j in 1:ncol(dataForHm)) {
    dataForHm[i,j] = ifelse(surv_resdf$p_PTVvar[surv_resdf$ptv_type == rownames(dataForHm)[i] & surv_resdf$cancer_type == colnames(dataForHm)[j]] < 0.05, surv_resdf$coef_PTVvar[surv_resdf$ptv_type == rownames(dataForHm)[i] & surv_resdf$cancer_type == colnames(dataForHm)[j]], NA)
  }
}
...
for(i in 1:nrow(dataForHmP)) {
  for(j in 1:ncol(dataForHmP)) {
    dataForHmP[i,j] = ifelse(surv_resdf$p_PTVvar[surv_resdf$ptv_type == rownames(dataForHmP)[i] & surv_resdf$cancer_type == colnames(dataForHmP)[j]] < 0.05, round(surv_resdf$p_PTVvar[surv_resdf$ptv_type == rownames(dataForHmP)[i] & surv_resdf$cancer_type == colnames(dataForHmP)[j]], digits = 4), NA)
  }
}

png(filename = "plots/Survival/surv_cox_heatmap.png", width = 30, height = 20, units = "cm", res = 300)
...
#Loads
ukb_data = readRDS("objects/ukb_data.rds")
load("C:/Users/bagil/Desktop/MyGit/ukb_tumor/objects/ptvb_antproc")

ukb_data %<>% left_join(ptvb_antproc, by = "eid")
...
res = pblapply(ptvs, function(p) {
  ...
  tumors_survres = pblapply(tumor_freq_data$cancer_type, function(t) {
    ...
    colnames(tempdfU)[5] = "PTVvar"
    if (length(table(tempdfU$PTVvar[tempdfU$status == 1])) > 1 & length(unique(tempdfU$sex)) == 1){
      hr_model = survival::coxph(Surv(time, status) ~ PTVvar + gpca1 + gpca2 + gpca3 + gpca4 + gpca5 + gpca6 + gpca7 + gpca8 + gpca9 + gpca10, data = tempdfU)
    } else if(length(table(tempdfU$PTVvar[tempdfU$status == 1])) == 1 & length(unique(tempdfU$sex)) == 1) { 
      hr_model = survival::coxph(Surv(time, status) ~  gpca1 + gpca2 + gpca3 + gpca4 + gpca5 + gpca6 + gpca7 + gpca8 + gpca9 + gpca10, data = tempdfU)
    } else if (length(table(tempdfU$PTVvar[tempdfU$status == 1])) > 1 & length(unique(tempdfU$sex)) == 2){
      hr_model =  survival::coxph(Surv(time, status) ~ PTVvar + sex + gpca1 + gpca2 + gpca3 + gpca4 + gpca5 + gpca6 + gpca7 + gpca8 + gpca9 + gpca10, data = as.data.frame(tempdfU))
    } else {
      hr_model =  survival::coxph(Surv(time, status) ~ sex + gpca1 + gpca2 + gpca3 + gpca4 + gpca5 + gpca6 + gpca7 + gpca8 + gpca9 + gpca10, data = as.data.frame(tempdfU))
    }
  forestmodel::forest_model(hr_model)
  })
  dev.off()
})
rm(i)
...
res_surv = pblapply(ptvs, function(p) {
  i=i+1
  .GlobalEnv$i = i
  pdf_filename = paste0(out_dir, "surv_plot", p, ".pdf")
  pdf(pdf_filename, onefile = T, width = pdf_width, height = pdf_height)
...
res = future_lapply(ptvs, function(p) {
  ...
  tumors_survres = pbsapply(tumor_freq_data$cancer_type, function(t) {
    ...
    tempdfU %<>% dplyr::select(eid, time, event, sex, all_of(p), gpca1, gpca2, gpca3, gpca4, gpca5, gpca6, gpca7, gpca8, gpca9, gpca10)
    colnames(tempdfU)[5] = "PTVvar"
    res.surv = coxph(Surv(time, event) ~ PTVvar + sex + gpca1 + gpca2 + gpca3 + gpca4 + gpca5 + gpca6 + gpca7 + gpca8 + gpca9 + gpca10, data = tempdfU)
    coxPvalues = summary(res.surv)$coefficients[,"Pr(>|z|)"]
    coxCoefs = summary(res.surv)$coefficients[,"coef"]
    names(coxPvalues)[grep("PTV", names(coxPvalues))] = "PTVvar"
    names(coxCoefs)[grep("PTV", names(coxCoefs))] = "PTVvar"
...
surv_resdf = bind_rows(res)
save(surv_resdf, file = "objects/res_surv_overall")
...
png(filename = "plots/Survival/surv_cox_overall_heatmap.png", width = 30, height = 20, units = "cm", res = 300)
hm = ComplexHeatmap::Heatmap(matrix = dataForHm, 
             name = "Coefficient", 
             na_col = "gray",
             col = colorRamp2(c(-2, 0, 2), c("green", "white", "red")),
...
res = future_lapply(ptvs, function(p) {
  ...
  tumors_survres = pbsapply(tumor_freq_data$cancer_type, function(t) {
    ...
    tempdfU %<>% dplyr::select(eid, time, status, sex, all_of(p), gpca1, gpca2, gpca3, gpca4, gpca5, gpca6, gpca7, gpca8, gpca9, gpca10)
    colnames(tempdfU)[5] = "PTVvar"
    if (length(unique(tempdfU$sex)) == 1){
    hr_model = survival::coxph(Surv(time, status) ~ PTVvar + gpca1 + gpca2 + gpca3 + gpca4 + gpca5 + gpca6 + gpca7 + gpca8 + gpca9, data = tempdfU)
    } else if (length(unique(tempdfU$sex)) == 2){
    hr_model =  survival::coxph(Surv(time, status) ~ PTVvar + sex + gpca1 + gpca2 + gpca3 + gpca4 + gpca5 + gpca6 + gpca7 + gpca8 + gpca9 + gpca10, data = tempdfU)
  } 
  forest = ggforest(hr_model, data = as.data.frame(tempdfU), main = paste0("Hazard ratio of ", t, " Group:", p), refLabel = "reference")
  print(forest)
  })
  dev.off()
})
...
res_surv = pblapply(ptvs, function(p) {
  i=i+1
  .GlobalEnv$i = i
  pdf_filename = paste0(out_dir, "surv_plot_overlall_", p, ".pdf")
  pdf(pdf_filename, onefile = T, width = pdf_width, height = pdf_height)
  tumors_survres = pbsapply(tumor_freq_data$cancer_type, function(t) {
    tumorous_d = tumorous_all %>% 
      filter(cancer_type == t & death == T) %>% 
...
ukb_data = readRDS("objects/ukb_data.rds")
load("C:/Users/bagil/Desktop/MyGit/ukb_tumor/objects/ptvb_antproc")
colnames(ptvb_antproc)[2] = "PTVantipr"
...
for(i in 1:nrow(dataForHm)) {
  for(j in 1:ncol(dataForHm)) {
    dataForHm[i,j] = ifelse(surv_resdf$p_metastati_tumorTRUE[surv_resdf$ptv_type == rownames(dataForHm)[i] & surv_resdf$cancer_type == colnames(dataForHm)[j]] < 0.05, surv_resdf$coef_PTVvar[surv_resdf$ptv_type == rownames(dataForHm)[i] & surv_resdf$cancer_type == colnames(dataForHm)[j]], NA)
  }
}
...
for(i in 1:nrow(dataForHmP)) {
  for(j in 1:ncol(dataForHmP)) {
    dataForHmP[i,j] = ifelse(surv_resdf$p_metastati_tumorTRUE[surv_resdf$ptv_type == rownames(dataForHmP)[i] & surv_resdf$cancer_type == colnames(dataForHmP)[j]] < 0.05, round(surv_resdf$p_metastati_tumorTRUE[surv_resdf$ptv_type == rownames(dataForHmP)[i] & surv_resdf$cancer_type == colnames(dataForHmP)[j]], digits = 4), NA)
  }
}
