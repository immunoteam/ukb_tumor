...
plotlist_PTV3 = lapply(X = unique(icd_mapping$cancer_type), FUN = function(x){
  ...
  unique_tempdf$event_indicator = ifelse(unique_tempdf$cancer_type == x, 1, 0)
  surv_obj = with(unique_tempdf, Surv(time_in_days, event = event_indicator))
  surv_model = survfit(surv_obj ~ PTV_burden_group_3, data = unique_tempdf)
  figure = ggsurvplot(surv_model, data = unique_tempdf, pval = T, title = paste0("Incidence Analysis for ", x, "\n(N=", nrow(tempdf_x), ")"))$plot
  figure = figure + ylim(1 - (nrow(tempdf_x) / nrow(unique_tempdf)) - (nrow(tempdf_x) / nrow(unique_tempdf)) * 0.05, 1)
  figure
  })

ggarrange(plotlist =  plotlist_PTV3, ncol = 5, nrow = 5, common.legend = T)
ggsave(filename = "C:/Users/bagil/Desktop/MyGit/ukb_tumor/plots/Survival/PTV_burden_g3_surv.jpg", width = 80, height = 40, units = "cm", dpi = 300)
...
plotlist_PTV2 = lapply(X = unique(icd_mapping$cancer_type), FUN = function(x){
  ...
  unique_tempdf$event_indicator = ifelse(unique_tempdf$cancer_type == x, 1, 0)
  surv_obj = with(unique_tempdf, Surv(time_in_days, event = event_indicator))
  surv_model = survfit(surv_obj ~ PTV_burden_group_2, data = unique_tempdf)
  figure = ggsurvplot(surv_model, data = unique_tempdf, pval = T,title = paste0("Incidence Analysis for ", x, "\n(N=", nrow(tempdf_x), ")"))$plot
  figure = figure + ylim(1 - (nrow(tempdf_x) / nrow(unique_tempdf)) - (nrow(tempdf_x) / nrow(unique_tempdf)) * 0.05, 1)
  figure
  })

ggarrange(plotlist =  plotlist_PTV2, ncol = 5, nrow = 5, common.legend = T)
ggsave(filename = "C:/Users/bagil/Desktop/MyGit/ukb_tumor/plots/Survival/PTV_burden_g2_surv.jpg", width = 80, height = 40, units = "cm", dpi = 300)
...
plotlist_PTV1 = lapply(X = unique(icd_mapping$cancer_type), FUN = function(x){
  ...
  unique_tempdf$event_indicator = ifelse(unique_tempdf$cancer_type == x, 1, 0)
  surv_obj = with(unique_tempdf, Surv(time_in_days, event = event_indicator))
  surv_model = survfit(surv_obj ~ PTV_burden_group, data = unique_tempdf)
  figure = ggsurvplot(surv_model, data = unique_tempdf, pval = T,title = paste0("Incidence Analysis for ", x, "\n(N=", nrow(tempdf_x), ")"))$plot
  figure = figure + ylim(1 - (nrow(tempdf_x) / nrow(unique_tempdf)) - (nrow(tempdf_x) / nrow(unique_tempdf)) * 0.05, 1)
  figure
  })

ggarrange(plotlist =  plotlist_PTV1, ncol = 5, nrow = 5, common.legend = T)
ggsave(filename = "C:/Users/bagil/Desktop/MyGit/ukb_tumor/plots/Survival/PTV_burden_g1_surv.jpg", width = 80, height = 40, units = "cm", dpi = 300)
