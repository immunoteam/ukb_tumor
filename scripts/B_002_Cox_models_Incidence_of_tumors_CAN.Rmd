...
res = pblapply(ptvs, function(p) {
  ...
  tumors_coxres = pbsapply(tumors, function(t) {
    ...
    #ggsave(paste0("Plots/002/", t, "/cox_fp_", p, ".jpg"), plot = fig, width = 30, height = 20, units = "cm", dpi = 150)
...
```{r}
load("Objects/overrep_ft_tsgs_tumors")
...
res = pblapply(ptvs, function(p) {
  ...
  tumors_coxres = pbsapply(tumors, function(t) {
    ...
    fig = forest_model(model = res.cox)
    ggsave(paste0("Plots/002/", t, "/cox_fp_", p, "_freqtsgs.jpg"), plot = fig, width = 20, height = 30, units = "cm", dpi = 150)
    as.numeric(rbind(summary(res.cox)$coefficients[,"coef"], summary(res.cox)$coefficients[,"Pr(>|z|)"]))
  })
  ...
  colnames(out)[seq(5,ncol(out),2)] = c("coef_PTVvar", "coef_sexmale", paste0("coef_gpca", 1:10), paste0("coef_ensg", 1:(((ncol(out)-12)/2)/2)))
  colnames(out)[seq(6,ncol(out),2)] = c("p_PTVvar", "p_sexmale", paste0("p_gpca", 1:10), paste0("p_ensg", 1:(((ncol(out)-12)/2)/2)))
