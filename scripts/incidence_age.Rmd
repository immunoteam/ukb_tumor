...
#Age-sex plots
```{r}
load("C:/Users/bagil/Desktop/ukb/objects/age_table_sex")
...
ggsave(filename = "C:/Users/bagil/Desktop/ukb/plots/Incidence/age_incidence_sex.jpg",  width = 80, height = 40, units = "cm", dpi = 300)
...
#Skin plot
```{r}
load("C:/Users/bagil/Desktop/ukb/objects/age_table_skin")
...
ggsave(filename = "C:/Users/bagil/Desktop/ukb/plots/Incidence/age_incidence_by_skin.jpg", width = 60, height = 30, units = "cm", dpi = 300)
...
ages_df_ethnic_temp = lapply(unique(ages_df_ethnic$ethnic_group), FUN = function(x){
  ...
  tempdf$cumulative_cases[is.na(tempdf$cumulative_cases)] = 0
  l = length(ukb_data_cancer$eid[ukb_data_cancer$ethnic_group == x])
  tempdf$n.people = l - tempdf$cumulative_cases
  tempdf
})
...
load("C:/Users/bagil/Desktop/ukb/objects/age_ethnic")
...
ggsave(filename = "C:/Users/bagil/Desktop/ukb/plots/Incidence/age_incidence_ethnic_f20.jpg", plot = ethnic_plots, width = 80, height = 40, units = "cm", dpi = 300)
...
ukb_data_cancer$PTV_burden = cut(ukb_data_cancer$PTV_burden_of_MAF_1_percent, breaks = quantile(ukb_data_cancer$PTV_burden_of_MAF_1_percent, probs = c(0, 0.5, 1), na.rm = T), labels = c("Low", "High"), include.lowest = T, right = T)
...
ages_df_burden = ukb_data_cancer %>%
  distinct(eid, age, PTV_burden) %>%
  group_by(age, PTV_burden) %>%
  summarize(Number_of_People = n()) %>%
  arrange(age) %>%
  group_by(PTV_burden) %>% 
  mutate(
...
temp_df = expand.grid(age = 1:max(ukb_data_cancer$age), PTV_burden = levels(ukb_data_cancer$PTV_burden))

ages_df_burden_complete = temp_df %>%
  left_join(ages_df_burden, by = c("age", "PTV_burden")) %>%
  mutate(Number_of_People = ifelse(is.na(Number_of_People), 0, Number_of_People))

ages_df_burden_temp = lapply(unique(ages_df_burden$PTV_burden), FUN = function(x){
  tempdf = ages_df_burden_complete[ages_df_burden_complete$PTV_burden == x, ]
  for(i in 2:nrow(tempdf)){
    if(is.na(tempdf$cumulative_cases[i]) & !is.na(tempdf$cumulative_cases[i - 1])){
      tempdf$cumulative_cases[i] = tempdf$cumulative_cases[i - 1]
    }
  ...
  tempdf$cumulative_cases[is.na(tempdf$cumulative_cases)] = 0
  l = length(ukb_data_cancer$eid[ukb_data_cancer$PTV_burden == x])
  tempdf$n.people = l - tempdf$cumulative_cases
  tempdf
})
...
age_datas_burden = lapply(X = unique(icd_mapping$cancer_type), FUN = function(x){ 
  ...
  unique_tempdf = rbind(tempdf_x, tempdf_no_x)
  unique_tempdf$diag_age_in_years = floor(as.numeric(unique_tempdf$age)/365.25)
  unique_tempdf$PTV_burden[is.na(unique_tempdf$PTV_burden)] = "No data"
  unique_tempdf %>%
    filter(cancer_type == x) %>%
    group_by(PTV_burden, diag_age_in_years) %>%
    summarise(number_of_cases = n()) %>%
    group_by(PTV_burden) %>%
    mutate(cumulative_cases_diag = cumsum(number_of_cases)) %>% 
    mutate(cancer_type = x, .before = "diag_age_in_years") 
...
age_table_burden_final = age_table_burden %>% 
  left_join(ages_df_Burden_complete_2, by = c("diag_age_in_years" = "age", "PTV_burden"))
...
load("C:/Users/bagil/Desktop/ukb/objects/age_table_burden")
age_burden_list = lapply(X = unique(icd_mapping$cancer_type), FUN = function(x){
  ...
  geom_point(mapping = aes(x = diag_age_in_years, y = age_associated_incidence, color = factor(PTV_burden))) +
  geom_smooth(mapping = aes(x = diag_age_in_years, y = age_associated_incidence, color = factor(PTV_burden))) +
  ggtitle(label = "Age associated incidence", x) + xlab("Diagnosis age") + ylab("Incidence")
})

ggarrange(plotlist = age_burden_list, ncol = 5, nrow = 5, common.legend = T)
ggsave(filename = "C:/Users/bagil/Desktop/ukb/plots/Incidence/age_incidence_gburden.jpg",  width = 80, height = 40, units = "cm", dpi = 300)
