...
#ICD mapping
```{r}
icd_oral=data.frame(ICD10 = c(paste0("C0", 1:9), paste0("C", 10:14)), cancer_type = "Lip,Oral_Cavity,Pharynx")
icd_colon = data.frame(ICD10 = paste0("C", 18:19), cancer_type = "Colon")
icd_rectum=data.frame(ICD10= paste0("C", 19:21), cancer_type= "Rectum")
icd_other_D_organs = data.frame(ICD10 = c(paste0("C", 23:24), paste0("C", 26)), cancer_type = "Other_Digestive_Organ")
icd_other_r_organs = data.frame(ICD10 = c("C30", "C21", "C33", paste0("C", 37:39)), cancer_type = "Other_Respiratory_and_Intrathoracic_Organs")
...
icd_only_datas=data.frame(ICD10 =c("C32", "C15", "C16", "C17", "C20", "C21", "C22", "C25", "C34", "C61", "C62", "C67", "C50", "C53", "C54", "C56", "C81", "C45", "C43","C73", "C96", "C88", "C90" ),
                          cancer_type = c("Larynx", "Esophagus", "Stomach", "Small_Intestine", "Rectum", "Anus","Liver", "Pancreas", "Lung", "Prostate", "Testis", "Bladder","Breast",  "Cervix", "Endometrium", "Ovary", "Hodgkin lymphoma", "Mesothelioma", "Melanoma", "Thyroid", "Non-Hodgkin lymphoma", "Multiple myeloma", "Multiple myeloma"))
...
```{r}
TCGA_ICD10 = read_xlsx("C:/Users/bagil/Desktop/ukb/TCGA_ICD10.xlsx", col_names = T)
...
histologydf = read_xlsx("C:/Users/bagil/Desktop/ukb/sitetype.icdo3.d20220429.xlsx")
load("C:/Users/bagil/Desktop/MyGit/ukb_tumor/objects/icd_mapping")
load("C:/Users/bagil/Desktop/ukb/objects/cancer_data_1")
icd_df = read_xlsx("C:/Users/bagil/Desktop/ukb/rawdata/icd10_codes.xlsx")
tumor_hist = cancer_data %>% 
  select(eid, ICD10, histology, ICD_hist)
icd_df = icd_df %>% 
  select(1:3) 
icd_df$`ICD-10 4th Edition - All C Codes are Malignant Neoplasms` = gsub("\\.", "", icd_df$`ICD-10 4th Edition - All C Codes are Malignant Neoplasms`)


tumor_hist$hist_short = substr(tumor_hist$histology, 1, 3)
tumor_hist$hist_type = histologydf$`Histology Description`[fmatch(tumor_hist$hist_short, histologydf$Histology)]

histologydf$`Histology/Behavior` = strsplit(histologydf$`Histology/Behavior`, "/")
histologydf$`Histology/Behavior` = sapply(histologydf$`Histology/Behavior`, function(x) x[1])
tumor_hist$hist_behavior = histologydf$`Histology/Behavior Description`[fmatch(tumor_hist$histology, histologydf$`Histology/Behavior`)]

tumor_hist$TCGAtumor_type = icd_df$`Cancer Waiting Times Site specific group`[fmatch(tumor_hist$ICD10, icd_df$`ICD-10 4th Edition - All C Codes are Malignant Neoplasms`)]
tumor_hist$TCGA_tumors = paste(tumor_hist$TCGAtumor_type, tumor_hist$hist_behavior, sep = "_")
tumor_hist$TCGA_tumors = gsub(" ", "_", tumor_hist$TCGA_tumors)
save(tumor_hist, file = "C:/Users/bagil/Desktop/MyGit/ukb_tumor/objects/tumor_hist_df")
