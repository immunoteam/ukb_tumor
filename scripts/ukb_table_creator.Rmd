...
#UKB datas
ukb_data_cancer_raw$PTV_burden = genetic_PTV$PTV_burden[fmatch(ukb_data_cancer_raw$eid, genetic_PTV$eid)]
...
#Cancer datas
cancer_data_raw$PTV_burden = genetic_PTV$PTV_burden[fmatch(cancer_data_raw$eid, genetic_PTV$eid)]
...
colnames(ptvb)[c(1,2)] = c("eid", "PTV_burden")

cancer_data_raw$eid = as.character(cancer_data_raw$eid)
cancer_data_raw %<>% 
  left_join(ptvb, by = "eid", multiple = "all")

ukb_data_cancer_raw$eid = as.character(ukb_data_cancer_raw$eid)

ukb_data_cancer_raw %<>% 
  left_join(ptvb, by = "eid")

save(cancer_data_raw, file = "C:/Users/bagil/Desktop/MyGit/ukb_tumor/objects/cancer_data_final_rawPTV")
save(ukb_data_cancer_raw, file = "C:/Users/bagil/Desktop/MyGit/ukb_tumor/objects/ukb_data_cancer_rawPTV")
...
ukb_data_cancer_PCA_rawPTV = ukb_data_cancer_raw 
cancer_data_PCA_rawPTV = cancer_data_raw
...
ukb_data_cancer = ukb_data_cancer %>%
  ...
  mutate(PTV_universal_2 = cut(PTV_burden, breaks = q_all[c("0%", "20%", "80%", "100%")], include.lowest = T)) %>% 
...
#PTV group 0-90%
ukb_data_cancer = ukb_data_cancer %>%
  mutate(PTV_uni_groups_1 = cut(PTV_burden, breaks = q_all[c("0%", "90%", "100%")], include.lowest = T)) %>% 
  mutate(PTV_uni_groups_2 = cut(PTV_burden, breaks = q_all[c("0%", "95%", "100%")], include.lowest = T))
...
#Add to ukb
#ukb_data_cancer = ukb_data_cancer %>%
  #mutate(PTV_cancer_1 = cut(PTV_burden, breaks = q_cancer[c("0%","10%", "90%", "100%")], include.lowest = T)) %>% 
  #mutate(PTV_cancer_2 = cut(PTV_burden, breaks = q_cancer[c("0%", "20%", "80%", "90%", "100%")],include.lowest = T)) %>% 
  #mutate(PTV_universal_3 = cut(PTV_burden, breaks = q_all[c("0%", "5%", "95%", "100%")], include.lowest = T))
...
cancer_data_list = lapply(X = tumors, FUN = function(t){
  ...
  tempdf %>% 
    mutate(PTV_cancer_type_1 = cut(PTV_burden,
                            breaks = unique(q_tumor_spec[[t]][c("0%","10%", "90%", "100%")]),
                            include.lowest = TRUE)) %>% 
    mutate(PTV_cancer_type_2 = cut(PTV_burden,
                            breaks = unique(q_tumor_spec[[t]][c("0%","20%", "60%", "100%")]),
                            include.lowest = TRUE)) %>%
    mutate(PTV_cancer_type_3 = cut(PTV_burden,
                            breaks = unique(q_tumor_spec[[t]][c("0%","5%", "95%", "100%")]),
                            include.lowest = TRUE)) %>%
    mutate(PTV_canctype_groups_1 = cut(PTV_burden,
                            breaks = unique(q_tumor_spec[[t]][c("0%", "90%", "100%")]),
                            include.lowest = TRUE)) %>% 
    mutate(PTV_canctype_groups_2 = cut(PTV_burden,
                            breaks = unique(q_tumor_spec[[t]][c("0%", "95%", "100%")]),
                            include.lowest = TRUE)) %>%
    ungroup()
})
...
#ukb_data_no_cancer %<>% mutate(PTV_cancer_type_1 = NA, PTV_cancer_type_2 = NA, PTV_cancer_type_3 = NA, PTV_canctype_groups = NA )

#columns_to_convert = c("PTV_cancer_type_1", "PTV_cancer_type_2", "PTV_cancer_type_3", "PTV_canctype_groups")
...
#Add to ukb
ukb_data_cancer = ukb_data_cancer %>%
  mutate(PTVim_groups_1 = cut(PTVim,
                                 breaks = unique(q_im_all[c("0%", "90%", "100%")]), include.lowest = TRUE)) %>% 
    mutate(PTVim_groups_2 = cut(PTVim,
                                 breaks = unique(q_im_all[c("0%", "95%", "100%")]), include.lowest = TRUE)) %>% 
    ungroup()

cancer_data = cancer_data %>%
  mutate(PTVim_universal_1 = cut(PTVim, breaks = q_im_all[c("0%","10%", "90%", "100%")], include.lowest = T)) %>% 
  mutate(PTVim_universal_2 = cut(PTVim, breaks = q_im_all[c("0%", "20%", "80%", "90%", "100%")], include.lowest = T)) %>% 
  mutate(PTVim_universal_3 = cut(PTVim, breaks = q_im_all[c("0%", "5%", "95%", "100%")], include.lowest = T))
...
#PTVim groups
cancer_data$PTVim_group = cut(cancer_data$PTVim, breaks = c(0, 1, max(cancer_data$PTVim)), include.lowest = T, right = F)
ukb_data_cancer$PTVim = as.numeric(ukb_data_cancer$PTVim)
ukb_data_cancer$PTVim_group = cut(ukb_data_cancer$PTVim, breaks = c(0, 1, max(ukb_data_cancer$PTVim)), include.lowest = T, right = F)

#PTVtsg groups
cancer_data$PTVtsg_group = cut(cancer_data$PTVtsg, breaks = c(0, 1, max(cancer_data$PTVtsg)), include.lowest = T, right = F)
ukb_data_cancer$PTVtsg_group = cut(ukb_data_cancer$PTVtsg, breaks = c(0, 1, max(ukb_data_cancer$PTVtsg)), include.lowest = T, right = F)

#PTVpli groups
cancer_data$PTVpli_group = cut(cancer_data$PTVpli, breaks = c(0, 1, max(cancer_data$PTVpli)), include.lowest = T, right = F)
ukb_data_cancer$PTVpli_group = cut(ukb_data_cancer$PTVpli, breaks = c(0, 1, max(ukb_data_cancer$PTVpli)), include.lowest = T, right = F)
...
#Add to ukb
ukb_data_cancer = ukb_data_cancer %>%
  mutate(PTVtsg_group_1 = cut(PTVtsg, breaks = q_tsg_all[c("0%", "90%", "100%")], include.lowest = T)) %>% 
  mutate(PTVtsg_group_2 = cut(PTVtsg, breaks = q_tsg_all[c("0%", "95%", "100%")], include.lowest = T)) %>% 
  ungroup()

cancer_data = cancer_data %>%
  mutate(PTVtsg_universal_1 = cut(PTVtsg, breaks = q_tsg_all[c("0%","10%", "90%", "100%")], include.lowest = T)) %>% 
  mutate(PTVtsg_universal_2 = cut(PTVtsg, breaks = q_tsg_all[c("0%", "20%", "80%", "90%", "100%")], include.lowest = T)) %>% 
  mutate(PTVtsg_universal_3 = cut(PTVtsg, breaks = q_tsg_all[c("0%", "5%", "95%", "100%")], include.lowest = T))
...
#Add to ukb
ukb_data_cancer = ukb_data_cancer %>%
  mutate(PTVpli_group_1 = cut(PTVpli, breaks = q_pli_all[c("0%", "90%", "100%")], include.lowest = T)) %>% 
  mutate(PTVpli_group_2 = cut(PTVpli, breaks = q_pli_all[c("0%", "95%", "100%")], include.lowest = T)) %>% 
 ungroup()

cancer_data = cancer_data %>%
  mutate(PTVpli_universal_1 = cut(PTVpli, breaks = q_pli_all[c("0%","10%", "90%", "100%")], include.lowest = T)) %>% 
  mutate(PTVpli_universal_2 = cut(PTVpli, breaks = q_pli_all[c("0%", "20%", "80%", "90%", "100%")], include.lowest = T)) %>% 
  mutate(PTVpli_universal_3 = cut(PTVpli, breaks = q_pli_all[c("0%", "5%", "95%", "100%")], include.lowest = T))
